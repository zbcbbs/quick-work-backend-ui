...
/**
 * CRUD 核心函数
 * 使用多crud时，请在关联crud的组件处使用crud-tag进行标记，如：<jobForm :job-status="dict.job_status" crud-tag="job" />
...
function CRUD(options) {
  ...
  const defaultOptions = {
    ...
    props: {}, // 自定义扩展属性
    queryOnPresenterCreated: true, // 在主页准备完成时查询
    debug: false // 调试
  }
  options = mergeOptions(defaultOptions, options) // 合并覆盖配置
  ...
  const data = {
    ...
    loading: false, // 整体加载状态
    downloadLoading: false, // 下载和导出的加载状态
    delAllLoading: false // 删除的加载状态
  }
  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          ...
          crud.page.total = data.recordsTotal
          crud.data = data.data
    ...
    doDelete(data) {
      ...
      return crud.crudMethod.del(ids.join(',')).then(() => {
        if (delAll) {
    ...
    doExport() {
      ...
      download(crud.url + '/export', crud.getQueryParams()).then(result => {
    ...
    getQueryParams: function() {
      ...
      return {
        page: (crud.page.page - 1) * (crud.page.size),
        size: crud.page.size,
        sort: crud.sort,
        ...crud.query,
        ...crud.params
  ...
  const crud = Object.assign({}, data) // 数据和配置
  Vue.observable(crud) // 让CRUD对象成为Vue的可响应对象
  Object.assign(crud, methods) // 附加方法
  ...
  Object.assign(crud, {
    ...
    registerVM(type, vm, index = -1) {
      ...
      this.vms.splice(index, 1, vmObj) // 向 index 位置添加 1 个 vmObj 对象
    },
    /**
     * 取消注册组件实例
     * @param {*} vm 组件实例
      ...
      for (let i = this.vms.length - 1; i >= 0; i--) {
        ...
        if (this.vms[i].type === type && this.vms[i].vm === vm) {
          if (i < 4) { // 内置预留vm数
            ...
            this.vms.splice(i, 1) // 移除当前位置的一个 vmObj 对象
          }
          break
        }
      }
  ...
  Object.freeze(crud) // 冻结的对象禁止对其修改
  return crud
}
...
/**
 * 合并覆盖配置
 * @param {Object} src 默认配置
 * @param {Object} opts 输入配置
 */
  ...
  return {
    ...
    beforeCreate() {
      ...
      cruds.forEach(ele => {
        ...
        this.$crud[ele.tag] = ele // $crud 访问组件的 crud 属性，添加多个CRUD对象
...
function pagination() {
  return {
    ...
    beforeCreate() {
      this.crud = lookupCrud(this) // crud与当前组件绑定
...
function crud(options = {}) {
  ...
  return {
    ...
    beforeCreate() {
      this.crud = lookupCrud(this) // 会查找父组件的crud
