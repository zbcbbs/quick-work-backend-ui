              ...
              <el-table-column label="关联字典">
                <template slot-scope="scope">
                  <el-select v-model="data[scope.$index].dictName" filterable class="edit-input" clearable size="mini" placeholder="请选择">
                    <el-option v-for="item in dicts" :key="item.id" :label="item.name === '' ? item.remark : item.name" :value="item.code" />
                  </el-select>
                </template>
              </el-table-column>
            </el-table>
        ...
        <el-card class="box-card" shadow="never">
          ...
          <el-form ref="form" :model="form" :rules="rules" size="small" label-width="78px">
            ...
            <el-form-item label="邮箱" prop="email">
              ...
              <span style="color: #C0C0C0;margin-left: 10px;">类上面的作者邮箱</span>
            </el-form-item>
            <el-form-item label="注释说明" prop="comment">
              <el-input v-model="form.comment" style="width: 40%;" />
              <span style="color: #C0C0C0;margin-left: 10px;">类上面的注释</span>
            </el-form-item>
            <el-form-item label="模块名称" prop="moduleName">
              <el-input v-model="form.moduleName" style="width: 40%" />
              <span style="color: #C0C0C0;margin-left: 10px;">模块名称，必须是已存在的项目名称</span>
            </el-form-item>
            <el-form-item label="包路径" prop="pack">
              <el-input v-model="form.pack" style="width: 40%" />
              <span style="color: #C0C0C0;margin-left: 10px;">项目包路径，必须是项目中的包路径</span>
            </el-form-item>
            <el-form-item label="接口名称" prop="apiAlias">
              <el-input v-model="form.apiAlias" style="width: 40%" />
              <span style="color: #C0C0C0;margin-left: 10px;">接口名称，控制器和接口文档说明</span>
            </el-form-item>
            <el-form-item label="前端路径" prop="path">
              <el-input v-model="form.path" style="width: 40%" />
              <span style="color: #C0C0C0;margin-left: 10px;">前端src/views文件夹下的目录，不存在则创建</span>
            </el-form-item>
            <el-form-item label="接口路径" prop="apiPath">
              <el-input v-model="form.apiPath" style="width: 40%" />
              <span style="color:#C0C0C0;margin-left: 10px;">前端src/api文件夹下的目录，不存在则创建</span>
            </el-form-item>
            <el-form-item label="去表前缀" prop="prefix">
              <el-input v-model="form.prefix" placeholder="默认不去除表前缀" style="width: 40%" />
              <span style="color: #C0C0C0;margin-left: 10px;">默认不去除表前缀</span>
            </el-form-item>
            <el-form-item label="是否覆盖" prop="cover">
              ...
              <span style="color: #C0C0C0;margin-left: 10px;">谨防误操作，请慎重设置</span>
            </el-form-item>
          </el-form>
        </el-card>
      </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      form: { id: null, tableName: '', author: '', email: '', comment: '', pack: '', path: '', moduleName: '', cover: 'false', apiPath: '', prefix: '', apiAlias: null },
      rules: {
        author: [
  ...
  created() {
    ...
    this.tableName = this.$route.params.tableName // 获取路由中传递的表名
    this.$nextTick(() => {
      this.init()
      this.getGenConfig()
      getDicts().then(data => {
