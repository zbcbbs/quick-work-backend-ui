<template>
  <div v-loading="!show" element-loading-text="加载中..." :style="!show ? 'height: 500px' : 'height: 100%'" class="app-container">
    <div v-if="show">
  ...
  methods: {
    init() {
      initData(this.url, {}).then(data => {
        ...
        this.cpuInfo.xAxis.data.push(data.data.time)
        this.memoryInfo.xAxis.data.push(data.data.time)
        this.cpuInfo.series[0].data.push(parseFloat(data.data.cpu.used))
        this.memoryInfo.series[0].data.push(parseFloat(data.data.memory.usageRate))
      })
    }
  }
}
